import { useEffect, useState } from "react";
import type { DateFormat, DayFormat } from "../../types/clock";

type SettingsPageProps = {
  isOpen: boolean;
  isMeridiem: boolean;
  setIsMeridiem: React.Dispatch<React.SetStateAction<boolean>>;
  valOfDate: DateFormat;
  setValOfDate: React.Dispatch<React.SetStateAction<DateFormat>>;
  valOfDay: DayFormat;
  setValOfDay: React.Dispatch<React.SetStateAction<DayFormat>>;
};

const SettingsPage = ({
  isOpen,
  isMeridiem,
  setIsMeridiem,
  valOfDate,
  setValOfDate,
  valOfDay,
  setValOfDay,
}: SettingsPageProps) => {
  const [changedSelect, setChangedSelect] = useState<string | null>(null);
  const [buttonName, setButtonName] = useState("Delete all data");

  useEffect(() => {
    if (!changedSelect) {
      return undefined;
    }

    const timeoutId = window.setTimeout(() => {
      setChangedSelect(null);
    }, 500);

    return () => window.clearTimeout(timeoutId);
  }, [changedSelect]);

  const handleDeleteData = () => {
    if (!isMeridiem && valOfDate === "shortcut_name" && valOfDay === "shortcut") {
      return;
    }

    localStorage.clear();
    setIsMeridiem(false);
    setValOfDate("shortcut_name");
    setValOfDay("shortcut");
    setButtonName("Data deleted");

    window.setTimeout(() => {
      setButtonName("Delete all data");
    }, 3000);
  };

  return (
    <div id="sidebar" className={isOpen ? "open" : ""}>
      <h2 style={{ textAlign: "center", margin: "30px 0" }}>Settings</h2>
      <div className="setting">
        <label htmlFor="time-format">Time format</label>
        <select
          id="time-format"
          value={isMeridiem ? "12h" : "24h"}
          className={changedSelect === "time-format" ? "changed" : ""}
          onChange={(event) => {
            setChangedSelect("time-format");
            setIsMeridiem(event.target.value === "12h");
          }}
        >
          <option value="24h">24-H Format</option>
          <option value="12h">12-H Format</option>
        </select>
      </div>
      <div className="setting">
        <label htmlFor="date-format">Date settings</label>
        <select
          id="date-format"
          value={valOfDate}
          className={changedSelect === "date-format" ? "changed" : ""}
          onChange={(event) => {
            setChangedSelect("date-format");
            setValOfDate(event.target.value as DateFormat);
          }}
        >
          <option value="shortcut_name">Shortcut name</option>
          <option value="full_name">Full name</option>
          <option value="dd/mm/yyyy">DD/MM/YYYY</option>
          <option value="mm/dd/yyyy">MM/DD/YYYY</option>
          <option value="yyyy/mm/dd">YYYY/MM/DD</option>
          <option value="yyyy/dd/mm">YYYY/DD/MM</option>
        </select>
      </div>
      <div className="setting">
        <label htmlFor="day">Day</label>
        <select
          id="day"
          value={valOfDay}
          className={changedSelect === "day" ? "changed" : ""}
          onChange={(event) => {
            setChangedSelect("day");
            setValOfDay(event.target.value as DayFormat);
          }}
        >
          <option value="shortcut">Shortcut</option>
          <option value="full">Full</option>
          <option value="number">Number</option>
        </select>
      </div>

      <div className="delete-container">
        <button className="delete-btn" onClick={handleDeleteData}>
          {buttonName}
        </button>
      </div>
    </div>
  );
};

export default SettingsPage;

...
const SettingsPage = ({ isOpen, isMeridiem, setIsMeridiem, valOfDate, setValOfDate, valOfDay, setValOfDay }: SettingsPageProps) => {
  ...
  const [buttonName, setButtonName] = useState<string>('Delete all data');
  ...
  const handleDeleteData = () => {
    ...
    setTimeFormat('24h');
    setDateFormat('shortcut_name');
    setDayFormat('shortcut');
  ...
  return (
    <div id='sidebar' className={isOpen ? 'open' : ''}>
      ...
      <div className='delete-container'>
        <button className='delete-btn' onClick={handleDeleteData}>{buttonName}</button>
      </div>
    </div>
  )
}
